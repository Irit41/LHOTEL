...
import { BlueButton, GrayButton, Spinner } from "../../styles";
...
export default function Registration({ navigation }) {
    ...
    try {
      if (CheackFields()) {
        ...
        let result = await fetch(
          "http://proj13.ruppin-tech.co.il/AddNewCustomer",
          requestOptions
        );
        if (result) {
          SetLoading(true);
  ...
  const CreateUser = () => {
    // פונצקיה היוצרת אובייקט חדש מסוג משתמש ומחזירה אותו
  ...
  return (
    <ScrollView>
      ...
      <View>{loading ? null : <Spinner />}</View>
      <View style={styles.label}>
        ...
        <TextInput
          value={password}
          placeholder="Password, 9 or more letters"
          style={styles.TextInput}
        ...
        <TextInput
          value={re_Password}
          placeholder="Re-Password, 9 or more letters"
          secureTextEntry={true}
          style={styles.TextInput}
          onChangeText={(re_Password) => setRe_Password(re_Password)}
        ></TextInput>
        <View style={{ height: 10 }}></View>
      ...
      <TouchableOpacity onPress={Save_User}>
